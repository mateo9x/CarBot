...
public class FunFactGame {
    ...
    public static void draw(CarBot carBot, Message message) {
        final Language language = carBot.getMessageSource().getLanguage();
        final List<FunFactModel> funFacts = getFunFacts(language);
        Random random = new Random();
        int number = random.nextInt(funFacts.size());
        FunFactModel funFact = funFacts.get(number);
        String header = carBot.getMessageSource().getMessage(FUN_FACT_PROPERTY);
